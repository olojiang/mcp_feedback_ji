...
describe('registryLock', () => {
  ...
  it('blocks when another alive pid holds lock', () => {
    const { canAcquireRegistryLock } = require('../out/registryLock.js')
    const existing = { pid: 100, port: 48201, acquired_at: 1, workspaces: ['/a'] }
    const owner = { pid: 200, port: 48202, acquired_at: 2, workspaces: ['/b'] }
    assert.equal(canAcquireRegistryLock(existing, owner, (pid) => pid === 100, 5000), false)
  })

  it('writeServersBatch writes all workspace hashes under lock', () => {
    ...
    const result = writeServersBatch({
      ...
      readLock: () => null,
      writeLock: (lock) => locks.push(lock),
    ...
    assert.equal(locks[0].pid, 99)
    assert.deepEqual(locks[0].workspaces, ['/ws/a', '/ws/b'])
  })

  it('skips batch write when lock held by other alive pid', () => {
    const { writeServersBatch } = require('../out/registryLock.js')
    const result = writeServersBatch({
      workspaces: ['/ws/a'],
      info: { port: 48202, pid: 200, version: '2.5.1-ji.67', started_at: 1000 },
      projectHash: () => 'h1',
      readLock: () => ({ pid: 100, port: 48201, acquired_at: 1500, workspaces: ['/ws/a'] }),
      writeLock: () => {},
      writeServer: () => {},
      isAlive: (pid) => pid === 100,
      now: 2000,
